...
const test_hira2kata = () => {
    ...
    const failedCases = cases.filter(({ input, expected }) => hira2kata(input) !== expected);
...
const test_hira2kataAll = () => {
    ...
    const failedCases = cases.filter(({ input, expected }) => hira2kataAll(input) !== expected);
    if (failedCases.length <= 0) {
        return;
    }
    console.error('[Failed]: hira2kata()');
    for (const { input, expected } of failedCases) {
        console.error(`input: ${input}, expected ${expected}, but got ${hira2kataAll(input)}`);
    }
};
const test_kata2hira = () => {
    ...
    for (const { input, expected } of failedCases) {
        console.error(`input: ${input}, expected ${expected}, but got ${kata2hira(input)}`);
    }
};
const test_kata2hiraAll = () => {
    ...
    const failedCases = cases.filter(({ input, expected }) => kata2hiraAll(input) !== expected);
    if (failedCases.length <= 0) {
        return;
    }
    console.error('[Failed]: kata2hiraAll()');
    for (const { input, expected } of failedCases) {
        console.error(`input: ${input}, expected ${expected}, but got ${kata2hiraAll(input)}`);
    }
};
const main = () => {
    test_hira2kata();
